...
def train_RF(parameters,X_train,y_train,X_test,y_test):
    ...
    model = RandomForestRegressor(n_estimators=1500,min_samples_leaf=30,max_depth=40)
    k_fold = KFold(n_splits=5, random_state=True)
    ...
    print 'rmse is', RMSE
    filename = 'RF_model_1_small_v4_LESSFEATURES_v2.sav'
...
def delete_useless_columns(dataframe):
    useless_variables = ['Unnamed: 0', 'Unnamed: 0.1', 'srch_id','year','click_bool','position','booking_bool','count_hotel','prob_book','prob_click']
...
def main():
    ...
    testset_predicted = train_RF(5,X_train,y_train,X_test,y_test)
