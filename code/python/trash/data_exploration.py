...
def dataset_reduction(train):
    ...
    sel_user_id.to_csv('selected_train.csv')
...
def remove_outliers(df):
    ...
    '''
    indices_to_remove = df[(df['prop_starrating'] < 5) & (df['price_usd'] > 5000)].index.tolist()
...
def histogram_position_vs_booking(df):
    ...
    for pos in range(50):
        pos_list.append(pos)
        bookings_no.append(df[df['position']==pos]['booking_bool'].tolist().count(1))
        click_no.append(df[df['position']==pos]['click_bool'].tolist().count(1))
    fig, ax = plt.subplots()
    width = 0.35
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('results/hotel_position_vs_click_booking.png')
    plt.show()
...
def clicked_booked_histograms(df,clicked):
    ...
    for variable in hotel_descriptor_variables:
        ...
        if variable in ['prop_review_score','prop_location_score2','srch_query_affinity_score']:
...
def main():
    ...
    ## 8. Plot histograms for all variables
    # histogram_all(train)

    ## 9. Study how the missing data affected the click/booking
    #clicked_booked_histograms(train,False)
    #clicked_booked_histograms(train,True)

    ## 10. Boxplot of the data points, looking for outliers
    # plot_outliers(df)
