...
def fill3(df,testset):
    ...
    df.loc[df['prop_log_historical_price'] == 0] = np.nan

    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 0)].fillna(histprice_1star_median)
    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 1)].fillna(histprice_1star_median)
    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 2)].fillna(histprice_2star_median)
    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 3)].fillna(histprice_3star_median)
    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 4)].fillna(histprice_4star_median)
    df.loc[(df['prop_log_historical_price'] == 0) & (df['prop_starrating'] == 5)].fillna(histprice_5star_median)
...
def calling_functions(dataset, is_test):
    ...
    train = train.drop(['visitor_hist_starrating', 'visitor_hist_adr_usd'], 1)
...
def main():
    ...
    trset_test =  'data/unprocessed_trainingset_HEADER.csv'


    train = calling_functions(testset, False)
    print train.isnull().sum()
